...
console.log('VALIDATE:',values);
    ...
    //provide meta information for batch processing
    if(values.length === 1 && values[0] === 'features'){
        var features = {responseType:0};
        if(options.responseType){
            features.responseType = options.responseType;
        }
        features.name = options.name;
        return features;
    }
